...
# --- editor ---
if [ "$(id -u)" -ne 0 ] && command -v nvim > /dev/null 2>&1; then
  export EDITOR=nvim
...
if command -v fzf > /dev/null 2>&1; then
  ...
  bind -x '"\C-x\C-a": __falias_widget'
fi
...
if command -v bat > /dev/null 2>&1; then
  ...
  bathelp() { # help command with bat formatting
    "$@" --help 2>&1 | bat --plain --language=help
  }
  batail() { # tail -f with bat syntax highlighting
